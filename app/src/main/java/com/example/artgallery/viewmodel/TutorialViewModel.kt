...
class TutorialViewModel(application: Application) : AndroidViewModel(application) {
    private val repository = TutorialRepository(application)
    val allTutorials: LiveData<List<Tutorial>> = repository.getAllTutorials()

    fun getTutorialsByCategory(category: String): LiveData<List<Tutorial>> {
        return repository.getTutorialsByCategory(category)
    }

    fun insertTutorial(tutorial: Tutorial) = viewModelScope.launch {
        repository.insertTutorial(tutorial)
    }

    fun updateTutorial(tutorial: Tutorial) = viewModelScope.launch {
        repository.updateTutorial(tutorial)
    }

    fun deleteTutorial(tutorial: Tutorial) = viewModelScope.launch {
        repository.deleteTutorial(tutorial)
    }

    fun incrementViewCount(tutorialId: Long) = viewModelScope.launch {
        repository.incrementViewCount(tutorialId)
    }

    fun updateUserProgress(tutorialId: Long, progress: Int) = viewModelScope.launch {
        repository.updateUserProgress(tutorialId, progress)
    }
}
